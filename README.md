...
Pick a random game from the Steam store from yout terminal.
...
`random-steam-game` scrapes the Steam store search index, selects a random title, and validates it against the Steam API to ensure it's a real, released, playable game (skips demos, betas, prologues, and unreleased titles).
